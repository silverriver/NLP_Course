...
class NLUDataset(Dataset):
    def __init__(self, paths, tokz, cls_vocab, logger, max_lengths=2048):
        self.logger = logger
        self.data = NLUDataset.make_dataset(paths, tokz, cls_vocab, logger, max_lengths)

    @staticmethod
    def make_dataset(paths, tokz, cls_vocab, logger, max_lengths):
        ...
        for path in paths:
            with open(path, 'r', encoding='utf8') as f:
                ...
                for domain, label, utt, slots in lines:
    ...
    def __getitem__(self, idx):
        intent, utt, token_type = self.data[idx]
        return {"intent": intent, "utt": utt, "token_type": token_type}
