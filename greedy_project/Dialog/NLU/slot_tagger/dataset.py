...
class NLUDataset(Dataset):
    def __init__(self, paths, tokz, slot_vocab, logger, max_lengths=2048):
        self.logger = logger
        self.data = NLUDataset.make_dataset(paths, tokz, slot_vocab, logger, max_lengths)

    @staticmethod
    def make_dataset(paths, tokz, slot_vocab, logger, max_lengths):
        ...
        for path in paths:
            with open(path, 'r', encoding='utf8') as f:
                ...
                for domain, intent, utt, slots in lines:
                    utt = tokz.convert_tokens_to_ids(list(utt.replace(' ', ''))[:max_lengths])
                    slots = [slot_vocab[i] for i in slots.split()]
                    assert len(utt) == len(slots)
                    dataset.append([[tokz.cls_token_id] + utt + [tokz.sep_token_id], 
    ...
    def __getitem__(self, idx):
        utt, token_type, slot = self.data[idx]
        return {"utt": utt, "token_type": token_type, "slot": slot}
