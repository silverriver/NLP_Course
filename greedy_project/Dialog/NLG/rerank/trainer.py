...
class Trainer:
    ...
    def _eval_train(self, epoch):
        ...
        rerank_loss, rerank_acc, step_count = 0, 0, 0
        ...
        for i, data in TQDM:
            ...
            rerank_logits = self.model(input_ids=text, attention_mask=mask, token_type_ids=token_type).view([-1])

            batch_loss = self.criterion(rerank_logits, label).mean()
            batch_rerank_acc = ((rerank_logits > 0).long() == label).float().mean()

            full_loss = batch_loss / self.config.batch_split
            full_loss.backward()

            rerank_loss += batch_loss.item()
            rerank_acc += batch_rerank_acc.item()
            step_count += 1
            ...
            if (i + 1) % self.config.batch_split == 0:
                ...
                rerank_loss /= step_count
                rerank_acc /= step_count
                curr_step = self.optimizer.state[self.optimizer.param_groups[0]["params"][-1]]["step"]

                if self.rank in [-1, 0]:
                    self.train_writer.add_scalar('loss/rerank_loss', rerank_loss, curr_step)
                    self.train_writer.add_scalar('acc/rerank_acc', rerank_acc, curr_step)
                    self.train_writer.add_scalar('lr', self.lr_scheduler.get_last_lr()[0], curr_step)
                    TQDM.set_postfix({'rerank_loss': rerank_loss, 'rerank_acc': rerank_acc})
    ...
    def _eval_test(self, epoch, step):
        self.model.eval()
        with torch.no_grad():
            dev_rerank_loss = torch.tensor(0.0, dtype=torch.float32, device=self.device)
            dev_rerank_acc = torch.tensor(0.0, dtype=torch.float32, device=self.device)
            ...
            for data in self.valid_dataloader:
                ...
                rerank_logits = self.model(input_ids=text, attention_mask=mask, token_type_ids=token_type).view([-1])
                loss = self.criterion(rerank_logits, label).sum()
                dev_rerank_loss += loss
                
                batch_rerank_acc = ((rerank_logits > 0).long() == label).float().sum()

                dev_rerank_acc += batch_rerank_acc
                count += text.shape[0]

            if self.rank != -1:
                torch.distributed.all_reduce(dev_rerank_loss, op=torch.distributed.reduce_op.SUM)
                torch.distributed.all_reduce(dev_rerank_acc, op=torch.distributed.reduce_op.SUM)
                torch.distributed.all_reduce(count, op=torch.distributed.reduce_op.SUM)

            dev_rerank_loss /= count
            dev_rerank_acc /= count

            if self.rank in [-1, 0]:
                self.valid_writer.add_scalar('loss/rerank_loss', dev_rerank_loss.item(), step)
                self.valid_writer.add_scalar('acc/rerank_acc', dev_rerank_acc.item(), step)
                log_str = 'epoch {:>3}, step {}'.format(epoch, step)
                log_str += ', dev_rerank_loss {:>4.4f}'.format(dev_rerank_loss.item())
                log_str += ', dev_rerank_acc {:>4.4f}'.format(dev_rerank_acc.item())
