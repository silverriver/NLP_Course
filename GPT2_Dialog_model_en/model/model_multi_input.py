...
import json
from modified_transformers import GPT2LMHeadModel
from .filtering import top_k_top_p_filtering


class MultiInputModel(nn.Module):
    def __init__(self, config, tokz):
        super(MultiInputModel, self).__init__()
        self.config = config
        self.tokz = tokz

        self.transformer_module = GPT2LMHeadModel.from_pretrained(config.dialogpt_dir)
        self.transformer_module.resize_token_embeddings(len(tokz))

    def encode(self, x, x_seg_id=None, masked_selfatt=None):
        mask = self.get_attn_mask(x)
        return self.transformer_module(x, token_type_ids=x_seg_id, masked_selfatt=masked_selfatt, attention_mask=mask, get_rep=True)[0], mask

    def generate(self, enc_x):
        if hasattr(self.transformer_module, 'lm_head'):
            return self.transformer_module.lm_head(enc_x)
        else:
            return self.transformer_module.module.lm_head(enc_x)

    def decode(self, x, enc_contexts):
        mask = self.get_attn_mask(x)
        x = self.transformer_module(x, attention_mask=mask, enc_contexts=enc_contexts)[0]
        return x

    def get_attn_mask(self, x):
        mask = x.eq(self.tokz.pad_token_id).bool()
        return ~mask
    ...
    def beam_search(self, enc_contexts=[], return_beams=False):
        with torch.no_grad():
            ...
            prevs = torch.full((batch_size * self.config.beam_size, 1), fill_value=self.tokz.bos_token_id, dtype=torch.long,
                               device=device)
            ...
            diversity_penalty = torch.zeros((batch_size, len(self.tokz)), device=device)

            # zrs:
            repeat = [{} for i in range(batch_size * self.config.beam_size)]
            # **********
            for i in range(self.config.max_decode_len):
                mask = self.get_attn_mask(prevs)
                logits = self.transformer_module(prevs, attention_mask=mask, enc_contexts=beam_enc_contexts)[0]
                ...
                penalty = penalty.unsqueeze(-1).repeat(1, 1, len(self.tokz))
                ...
                if i == 0:
                    ...
                    for g in range(self.config.diversity_groups):
                        ...
                        g_idxs += g * group_size * len(self.tokz)

                        all_scores.append(g_scores)
                        all_idxs.append(g_idxs)

                        diversity_penalty.scatter_add_(1, torch.fmod(g_idxs, len(self.tokz)),
                    ...
                    beam_idxs = (idxs.float() / len(self.tokz)).long()
                ...
                sym_idxs[is_end] = self.tokz.pad_token_id
                beam_lens[~is_end] += 1
                is_end[sym_idxs == self.tokz.eos_token_id] = 1
                ...
                for b in range(batch_size * self.config.beam_size):
                    b_list = prevs_list[b]
                    if len(b_list) > 2 and b_list[-1] != self.tokz.pad_token_id and b_list[-1] != self.tokz.eos_token_id:
    ...
    def top_k_top_p_search(self, enc_contexts=[], top_k=0, top_p=0.0, filter_value=-float('Inf'),
        ...
        with torch.no_grad():
            ...
            prevs = torch.full((batch_size * sample_size, 1), fill_value=self.tokz.bos_token_id,
            ...
            for i in range(self.config.max_decode_len):
                ...
                next_token[is_end] = self.tokz.pad_token_id
                lens[~is_end] += 1
                is_end[next_token == self.tokz.eos_token_id] = 1
