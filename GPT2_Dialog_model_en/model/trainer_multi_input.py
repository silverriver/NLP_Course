...
class Trainer:
    def __init__(self, model, train_dataset, valid_dataset, config, log_dir, logger,
                 device=cuda_device, ignore_idxs=[], distributed=False):
        self.config = config
        self.device = device
        self.logger = logger
        ...
        self.lm_criterion = nn.CrossEntropyLoss(ignore_index=self.model.tokz.pad_token_id).to(device, non_blocking=True)
        self.ppl_criterion = nn.CrossEntropyLoss(reduction='none', ignore_index=self.model.tokz.pad_token_id).to(device, non_blocking=True)
        self.criterion = LabelSmoothingLoss(n_labels=len(self.model.tokz), smoothing=config.label_smoothing,
                                            ignore_index=self.model.tokz.pad_token_id).to(device, non_blocking=True)
        base_optimizer = Adam(self.model.parameters(), lr=config.lr, weight_decay=0.01)
        self.optimizer = NoamOpt(self.model.transformer_module.config.n_embd, config.lr, config.lr_warmup, base_optimizer)
        ...
        self.train_dataloader = DataLoader(train_dataset, batch_size=config.batch_size, sampler=self.train_sampler,
                                           ...
                                           collate_fn=PadBatchSeq(self.model.tokz.pad_token_id))
    ...
    def _eval_train(self, epoch):
        ...
        for i, data in ITER:
            ...
            context.masked_fill_(ignore_mask, self.model.tokz.pad_token_id)
    ...
    def _eval_test(self):
        ...
        with torch.no_grad():
            ...
            for i, data in ITER:
                ...
                pad_mask = ~nexts.view(-1).eq(self.model.tokz.pad_token_id).bool()
    ...
    def _pred_sample(self, n_sample):
        with torch.no_grad():
            ...
            samples = PadBatchSeq(self.model.tokz.pad_token_id)([self.valid_dataset[idx] for idx in samples_idxs])
            ...
            for j in range(len(samples_idxs)):
                ...
                post_str = self.model.tokz.decode_with_no_eos(post_str)
                resp_str = samples['resp'][j].tolist()
                resp_str = self.model.tokz.decode_with_no_eos(resp_str)
                pred_str = self.model.tokz.decode_with_no_eos(prediction[j])
    ...
    def ids_cut(self, ids, length=None):
        ...
        if type(ids) is not list:
            ids = ids.tolist()
        if length is None:
            length = ids.index(self.model.tokz.eos_token_id)
        return ids[1: length]
